...
function Simulator(
    ...
    Simulator(
        m, 
        β_subset, Σ_subset, σ²_subset, 
        Xβ, re_storage, re_dist,
...
function simulate!(
    ...
    @inbounds @views for bidx = 1:m.b
        ...
        rand!(simulator.re_dist, simulator.re_storage) # simulating random effects
        BLAS.gemv!('N', T(1), m.data[bidx].Z, simulator.re_storage, T(1), m.data[bidx].y) # y = Xβ + Zα + error
        # bidx == 1 && print("m.data[bidx].y = ", m.data[bidx].y, "\n")
    end
end
